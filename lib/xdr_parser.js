...
class XdrParser extends Parser {
  constructor(input) {
    ...
    $.RULE('union_body', () => {
      ...
      $.OPTION(() => {
        $.CONSUME(Default); $.CONSUME(Colon);
        let decl = $.SUBRULE2($.declaration);
        members.push({ type: 'default', declaration: decl });
      });
      $.CONSUME(RBrace);
    ...
    $.RULE('type_def', () => {
      ...
      if (!!params && !!members) { // special case for unions
        return { type: type, name: name, params: params, members: members };
      }
